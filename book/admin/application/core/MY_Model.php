...
Class MY_Model extends CI_Model
{
   ...
   public function encode_id($id) {
	...
	public function get_ascii_db($st,$tbl_name,$field){
        ...
        $vowels_ = array('\"');
        $cf = preg_replace('!\s+!', ' ', $dtr);
        $name_ascii = $this->get_slug(str_replace($vowels_, ' ', $dtr));
		
		$dem = 0;
		$i = 1;
		$data = $name_ascii;
		while ($dem == 0) {
			$db = $this->db->query('SELECT id FROM '.$tbl_name.' WHERE '.$field.' = "'.$data.'" ')->num_rows();
			if ( $db == 0 ){ 
				$dem = 1;
			}else{ 
				$data = $name_ascii.' '.$i;
