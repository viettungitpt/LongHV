...
Class Mnxb extends MY_Model
{
	...
	public function getNhaxuatbanDB($number,$offset,$txtsearch)
    {
        $limit = $offset ? intval($offset) : '0';
        $sql = "SELECT * FROM nhaxuatban WHERE 1=1 ";
		if( !in_array($txtsearch,array('',null)) ){
			$sql .= ' AND tenNhaXuatBan LIKE "%'.$txtsearch.'%" '; 
		}
		
        $sql_limit = " ORDER BY maNhaXuatBan DESC LIMIT ".$limit.",".$number."";
	...
	public function deleteNhaXuatBanDb($id){
		$this->db->query('DELETE FROM nhaxuatban WHERE maNhaXuatBan = '.$id);
		$this->db->query('DELETE FROM sanpham WHERE id_MaNhaXuatBan = '.$id);
	}
	
	public function addNhaXuatBanDb($tenNhaXuatBan)
    {
        $arr = array(
                'tenNhaXuatBan' => $tenNhaXuatBan
            );
        $this->db->insert('nhaxuatban', $arr);
        return 200;
    }
	
	public function getNhaxuatbanInfo($id){
		return $this->db->query('SELECT * FROM nhaxuatban WHERE maNhaXuatBan = '.$id)->row_object();
	}
	
	public function updateNhaXuatBanDb($tenNhaXuatBan,$id){
		$arr = array(
			'tenNhaXuatBan' => $tenNhaXuatBan
			);
		$this->db->update('nhaxuatban',$arr,'maNhaXuatBan = '.$id);
		return 200;
	}
	/////////////////////////////////////////////////////////////////////
