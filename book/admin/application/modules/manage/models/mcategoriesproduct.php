...
Class Mcategoriesproduct extends MY_Model
{
    ...
    public function getCategoriesProductDB($number,$offset)
    {
        $limit = $offset ? intval($offset) : '0';
        $sql  = '';
        $sql = "SELECT * FROM danhmuc WHERE 1 = 1 ORDER BY `thuTu` ASC";
	...
	public function deleteCategoriesProductDB($idArr){
		foreach ( $idArr AS $el=>$le ){
			$this->db->delete('categories', array('id' => $le, 'type' => 1)); 
		}
        return 200;
	}
	public function deleteCategoriesDb($id){
		$this->db->delete('danhmuc', array('maDanhMuc' => $id)); 
        return 200;
	}
	
	
	
	public function getInfoCategories($id){
		return $this->db->query("SELECT * FROM danhmuc WHERE maDanhMuc = ".$id."")->row_object();
	}
	
	public function editCategoriesDb($id,$tenDanhMuc,$thuTu,$kichHoat){
			$data = array(
        ...
        $this->db->update('danhmuc', $data, "maDanhMuc = ".$id);
        return 200;
	}
	
	public function addCategoriesDb($tenDanhMuc,$thuTu,$kichHoat){
		$data = array(
			'tenDanhMuc' => $tenDanhMuc,
			'thuTu' => $thuTu,
			'kichHoat' => $kichHoat,
